...
export default function MoviesPage() {
  ...
  const [rating, setRating] = useState(0);
    ...
    const handleAddMustWatch = async () => {
      ...
      try {
        setIsLoading(true);
        await addMovie({ title: newMustWatchTitle.trim(), date: today, rating: 0 });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4 sm:p-6">
        <div className="flex justify-center gap-6">
          ...
          <div className="w-96">
            <div className="bg-white dark:bg-gray-900 rounded-4xl p-6 shadow-2xl">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Must-Watches</h2>
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-xl transition-colors shadow-md"
                  >
                    {isLoading ? '...' : 'Hinzufügen'}
                  </button>
                </div>
                <div className="max-h-96 overflow-y-auto space-y-2">
                  {mustWatches.map(movie => (
                    <div key={movie.id} className="bg-gray-50 dark:bg-gray-800 p-3 rounded-xl shadow-sm flex justify-between items-center">
                      <p className="text-gray-900 dark:text-white flex-grow">{movie.title}</p>
                      <div className="flex items-center">
                        ...
                        <button onClick={() => handleDeleteMovie(movie.id!, movie.title)} className="text-red-500 hover:text-red-700 p-1 ml-2" title="Film löschen">
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                      </div>
                    </div>
